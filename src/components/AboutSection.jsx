...
const NeuralSphere = ({ isPushed }) => {
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      const coreRadius = baseRadius * 0.3 + Math.sin(elapsed * 3) * (baseRadius * 0.05);
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        const breathe = Math.sin(elapsed * 1.5 + p.pulseOffset) * (baseRadius * 0.03) + (wave > 0.8 ? baseRadius * 0.05 : 0);
      ...
      for (let i = 0; i < projectedPoints.length; i++) {
        ...
        for (let j = i + 1; j < projectedPoints.length; j++) {
          ...
          if (dist < Math.pow(baseRadius * 0.35 * p1.scale, 2)) {
            ctx.beginPath();
            ctx.moveTo(p1.px, p1.py);
            ctx.lineTo(p2.px, p2.py);

            if (p1.wave > 0.9 || p2.wave > 0.9) {
               ctx.strokeStyle = "rgba(0, 255, 200, 0.9)"; // Cyan pulse
            } else if (Math.random() > 0.998) {
               ctx.strokeStyle = "rgba(255, 0, 85, 0.8)"; // Magenta glitch
            } else {
               const alpha = Math.max(0.02, 0.4 - (p1.z + baseRadius) / (baseRadius * 2));
        ...
        // Draw Nodes
        const alpha = Math.max(0.1, 0.9 - (p1.z + baseRadius) / (baseRadius * 2));
        ctx.fillStyle = p1.wave > 0.9 ? "#00ffc8" : `rgba(0, 255, 65, ${alpha})`;
        ctx.beginPath();
        ctx.arc(p1.px, p1.py, Math.max(0.5, (p1.wave > 0.9 ? 3.5 : 2.5) * p1.scale), 0, Math.PI * 2);
        ctx.fill();
      }
  ...
  return (
    <div
      ...
      style={{
        ...
        filter: isPushed ? "blur(5px)" : "blur(0px)",
      }}
    >
...
const GyroscopeHologram = () => (
  <div className="relative w-40 h-40">
    {/* Glow Aura */}
    <div className="absolute inset-[-20%] rounded-full animate-pulse pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(0,255,255,0.12) 0%, transparent 70%)' }} />
    {/* Pulsing outer ring */}
    <div className="absolute inset-[-10%] rounded-full border border-cyan-500/20 animate-ping pointer-events-none" style={{ animationDuration: '3s' }} />
    {/* CSS rings */}
    <div className="absolute inset-2 preserve-3d" style={{ animation: "spin-y 8s linear infinite" }}>
      <div className="absolute inset-0 rounded-full border-2 border-cyan-400/60" style={{ boxShadow: '0 0 12px rgba(0,255,255,0.3)' }} />
      <div className="absolute inset-0 rounded-full border-2 border-cyan-400/60" style={{ transform: "rotateX(90deg)", boxShadow: '0 0 12px rgba(0,255,255,0.3)' }} />
      <div className="absolute inset-0 rounded-full border border-cyan-400/30" style={{ transform: "rotateY(90deg)" }} />
    </div>
    {/* Three.js core */}
...
const DataCubeHologram = () => (
  <div className="relative w-36 h-36">
    {/* Glow Aura */}
    <div className="absolute inset-[-20%] rounded-full animate-pulse pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(255,0,128,0.12) 0%, transparent 70%)' }} />
    {/* Pulsing outer ring */}
    <div className="absolute inset-[-10%] rounded-full border border-pink-500/20 animate-ping pointer-events-none" style={{ animationDuration: '4s' }} />
    {/* CSS cube */}
    <div className="absolute inset-2 preserve-3d" style={{ animation: "spin-cube 10s linear infinite" }}>
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "translateZ(28px)", boxShadow: '0 0 10px rgba(255,0,128,0.2)' }} />
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "rotateY(180deg) translateZ(28px)" }} />
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "rotateY(-90deg) translateZ(28px)" }} />
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "rotateY(90deg) translateZ(28px)" }} />
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "rotateX(90deg) translateZ(28px)" }} />
      <div className="absolute inset-0 border-2 border-pink-400/50 bg-pink-500/5" style={{ transform: "rotateX(-90deg) translateZ(28px)" }} />
    </div>
    {/* Three.js core */}
...
const StackedPanelsHologram = () => (
  <div className="relative w-36 h-36">
    {/* Glow Aura */}
    <div className="absolute inset-[-20%] rounded-full animate-pulse pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(255,200,0,0.12) 0%, transparent 70%)' }} />
    {/* Pulsing outer ring */}
    <div className="absolute inset-[-10%] rounded-full border border-yellow-500/20 animate-ping pointer-events-none" style={{ animationDuration: '3.5s' }} />
    {/* CSS panels */}
    <div className="absolute inset-2 preserve-3d" style={{ transform: "rotateX(55deg) rotateY(15deg)" }}>
      <div className="absolute inset-0 preserve-3d" style={{ animation: "spin-panels 6s linear infinite" }}>
        <div className="absolute inset-0 border-2 rounded-lg border-yellow-400/60" style={{ transform: "translateZ(30px)", boxShadow: '0 0 10px rgba(255,200,0,0.2)' }} />
        <div className="absolute inset-0 border-2 rounded-lg border-yellow-400/30" style={{ transform: "translateZ(0px) scale(0.8)" }} />
        <div className="absolute inset-0 border-2 rounded-lg border-yellow-400/15" style={{ transform: "translateZ(-30px) scale(0.6)" }} />
      </div>
    </div>
    {/* Three.js core */}
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 z-10">
  ...
  return (
    <>
      ...
      >
        ...
        {/* Floating Hex Data */}
        <div className="absolute top-[20%] right-[5%] font-mono text-[9px] text-green-500/10 pointer-events-none z-[5] leading-relaxed" style={{ animation: 'hex-float 6s ease-in-out infinite' }}>
          0xAF29E1B3<br/>0x00FF4100<br/>0xDEADBEEF
        </div>
        <div className="absolute bottom-[25%] right-[8%] font-mono text-[9px] text-cyan-500/8 pointer-events-none z-[5] leading-relaxed" style={{ animation: 'hex-float 8s ease-in-out infinite 2s' }}>
          NODE::ACTIVE<br/>PKT_RECV: 847<br/>LATENCY: 0.3ms
        </div>
        ...
        <div className="absolute inset-0 w-full h-full">
          ...
          <div
            ...
            style={{ transform: activeNode === "SKILLS" ? "translate(-50%, -50%) scale(1.5)" : undefined }}
          >
            <div
          ...
          <div
            ...
            onMouseLeave={() => setHoveredNode(null)}
            style={{ transform: activeNode === "BIOMETRICS" ? "translate(-50%, -50%) scale(1.5)" : undefined }}
          >
            <div
              className="relative flex items-center justify-center transition-transform duration-700"
            >
          ...
          <div
            ...
            onMouseLeave={() => setHoveredNode(null)}
            style={{ transform: activeNode === "DIRECTIVES" ? "translate(-50%, -50%) scale(1.5)" : undefined }}
          >
            <div
              className="relative flex items-center justify-center transition-transform duration-700"
            >
        ...
        <div className={`absolute inset-0 z-[60] flex items-center justify-center pointer-events-none ${activeNode ? "opacity-100" : "opacity-0"} transition-all duration-700`}>
          <div className="w-full max-w-7xl h-full flex flex-col md:flex-row items-center justify-between px-8 md:px-16 gap-8">

            {/* LEFT PANEL */}
            <div className={`w-full md:w-[35%] pointer-events-auto transition-all duration-700 ${activeNode ? "translate-x-0 opacity-100" : "-translate-x-12 opacity-0"}`}>
              <div className="border border-white/5 bg-black/60 backdrop-blur-xl rounded-lg p-6 space-y-5" style={{ boxShadow: '0 0 40px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
                {activeNode === "SKILLS" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-cyan-400 uppercase">System.Augmentations</span>
                      <span className="font-mono text-[9px] px-2 py-0.5 rounded-full border border-cyan-500/30 text-cyan-400 bg-cyan-500/10">ACTIVE</span>
                    </div>
                    <div className="w-full h-px bg-gradient-to-r from-cyan-500/40 via-cyan-500/10 to-transparent mb-4" />
                    <AugmentationBar name="REACT_CORE" level={95} colorClass="bg-cyan-400" />
                    <AugmentationBar name="NEXT.JS_FRAMEWORK" level={88} colorClass="bg-cyan-400" />
                    <AugmentationBar name="TAILWIND_CSS" level={98} colorClass="bg-cyan-400" />
                  </>
                )}
                {activeNode === "BIOMETRICS" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-pink-400 uppercase">Biometric.Readout</span>
                      <span className="font-mono text-[9px] px-2 py-0.5 rounded-full border border-pink-500/30 text-pink-400 bg-pink-500/10">VERIFIED</span>
                    </div>
                    ...
                    <div className="space-y-4 font-mono text-sm">
                      <div className="flex items-center gap-3 border-b border-pink-500/10 pb-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; DESIGNATION</span>
                        <span className="text-white font-medium">Muhammad Wasay</span>
                      </div>
                      <div className="flex items-center gap-3 border-b border-pink-500/10 pb-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; CLASS</span>
                        <span className="text-white font-medium">Full-Stack Engineer</span>
                      </div>
                      <div className="flex items-center gap-3 border-b border-pink-500/10 pb-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; SPECIALIZATION</span>
                        <span className="text-white font-medium">Frontend Architecture</span>
                      </div>
                    </div>
                  </>
                )}
                {activeNode === "DIRECTIVES" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-yellow-400 uppercase">Core.Directives</span>
                      <span className="font-mono text-[9px] px-2 py-0.5 rounded-full border border-yellow-500/30 text-yellow-400 bg-yellow-500/10">ARMED</span>
                    </div>
                    <div className="w-full h-px bg-gradient-to-r from-yellow-500/40 via-yellow-500/10 to-transparent mb-4" />
                    <div className="space-y-4 font-mono text-xs leading-relaxed">
                      <div className="flex gap-3 items-start">
                        <span className="text-yellow-500/80 shrink-0 mt-0.5">01</span>
                        <div><span className="text-white font-bold block mb-1">INTERFACE_SYNTHESIS</span><span className="text-yellow-500/50">Architecting high-performance, immersive web applications.</span></div>
                      </div>
                      <div className="flex gap-3 items-start">
                        <span className="text-yellow-500/80 shrink-0 mt-0.5">02</span>
                        <div><span className="text-white font-bold block mb-1">PERFORMANCE_OPTIMIZATION</span><span className="text-yellow-500/50">Maximizing frame rates and minimizing latency.</span></div>
                      </div>
                    </div>
                  </>
                )}
            ...
            {/* RIGHT PANEL */}
            <div className={`w-full md:w-[35%] pointer-events-auto transition-all duration-700 delay-100 ${activeNode ? "translate-x-0 opacity-100" : "translate-x-12 opacity-0"}`}>
              <div className="border border-white/5 bg-black/60 backdrop-blur-xl rounded-lg p-6 space-y-5" style={{ boxShadow: '0 0 40px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
                {activeNode === "SKILLS" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-cyan-400/60 uppercase">Extended.Modules</span>
                    </div>
                    <div className="w-full h-px bg-gradient-to-r from-cyan-500/20 to-transparent mb-4" />
                    <AugmentationBar name="NODE.JS_RUNTIME" level={85} colorClass="bg-cyan-400" />
                    <AugmentationBar name="THREE.JS_WEBGL" level={75} colorClass="bg-cyan-400" />
                    <AugmentationBar name="UI_UX_ARCHITECTURE" level={90} colorClass="bg-cyan-400" />
                  </>
                )}
                {activeNode === "BIOMETRICS" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-pink-400/60 uppercase">System.Vitals</span>
                    </div>
                    <div className="w-full h-px bg-gradient-to-r from-pink-500/20 to-transparent mb-4" />
                    <div className="space-y-4 font-mono text-sm">
                      <div className="flex items-center gap-3 border-b border-pink-500/10 pb-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; NEURAL_UPTIME</span>
                        <span className="text-white font-medium">5+ Years Active</span>
                      </div>
                      <div className="flex items-center gap-3 border-b border-pink-500/10 pb-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; PROJECTS</span>
                        <span className="text-white font-medium">30+ Deployed</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-pink-500/60 text-xs w-28 shrink-0">&gt; STATUS</span>
                        <span className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" /><span className="text-green-400 font-medium">OPTIMAL</span></span>
                      </div>
                    </div>
                  </>
                )}
                {activeNode === "DIRECTIVES" && (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-mono text-[10px] tracking-[0.4em] text-yellow-400/60 uppercase">Sub.Protocols</span>
                    </div>
                    <div className="w-full h-px bg-gradient-to-r from-yellow-500/20 to-transparent mb-4" />
                    <div className="space-y-4 font-mono text-xs leading-relaxed">
                      <div className="flex gap-3 items-start">
                        <span className="text-yellow-500/80 shrink-0 mt-0.5">03</span>
                        <div><span className="text-white font-bold block mb-1">AESTHETIC_INTEGRITY</span><span className="text-yellow-500/50">Upholding strict visual standards across every pixel.</span></div>
                      </div>
                      <div className="flex gap-3 items-start">
                        <span className="text-yellow-500/80 shrink-0 mt-0.5">04</span>
                        <div><span className="text-white font-bold block mb-1">CONTINUOUS_EVOLUTION</span><span className="text-yellow-500/50">Perpetual learning. Adapting to emerging technologies.</span></div>
                      </div>
                    </div>
                  </>
                )}
        ...
        {/* Click-anywhere hint */}
        <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-[65] font-mono text-[10px] tracking-[0.3em] text-white/20 transition-all duration-700 ${activeNode ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
